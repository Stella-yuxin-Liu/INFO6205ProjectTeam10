...
/**
 * This class represents the purest form of Husky Sort based on IntroSort for
 * pass 1 and the System sort for pass 2.
 * <p>
 * CONSIDER redefining all of the "to" parameters to be consistent with our
 * other Sort utilities.
 *
 * @param <X> the type of the elements to be sorted.
 */
public class PureHuskySorts<X extends Comparable<X>> {
	...
	public void sort(final X[] xs) {
		...
		// This is necessary if we might be sorting a pre-sorted array. Otherwise, we
	 ...
	 * @param mayBeSorted      if this is true, then we should perform a random
	 *                         shuffle to prevent an O(N*N) performance. NOTE: that
	 *                         even though we are using IntroSort, the random
	 *                         shuffle precaution is necessary when
	 * @param useInsertionSort if true, then insertion sort will be used to mop up
	 *                         remaining inversions instead of system sort.
	 */
	public PureHuskySorts(final HuskyCoder<X> huskyCoder, final boolean mayBeSorted, final boolean useInsertionSort) {
	...
	private void introSort(final X[] objects, final long[] longs, final int from, final int to,
		...
		if (longs[hi] < longs[lo])
			swap(objects, longs, lo, hi);

		int lt = lo + 1, gt = hi - 1;
		int i = lo + 1;
		while (i <= gt) {
			if (longs[i] < longs[lo])
				swap(objects, longs, lt++, i++);
			else if (longs[hi] < longs[i])
				swap(objects, longs, i, gt--);
			else
				i++;
		}
		...
		if (longs[lt] < longs[gt])
			introSort(objects, longs, lt + 1, gt, depthThreshold - 1);
	...
	private void downHeap(final X[] objects, final long[] longs, int i, final int n, final int lo) {
		...
		while (i <= n / 2) {
			child = 2 * i;
			if (child < n && longs[lo + child - 1] < longs[lo + child])
				child++;
			if (d >= longs[lo + child - 1])
				break;
	...
	/**
	 * Swap method for insertion sort which takes advantage of the known fact that
	 * the elements of the array at indices less than i are in order.
	 *
	 * @param xs    the X array.
	 * @param longs the long array.
	...
	// NOTE that we keep this false because, for the size of arrays that we need to
	// sort via insertion sort,
	// This optimization doesn't really help.
	// That might be because (a) arrays are short and (b) the binary search will
	// likely take quite a bit longer than
	// necessary when the array is already close to being in order (since binary
	// search starts in the middle).
	// It would be like looking up aardvark in the dictionary using strict binary
	// search.
